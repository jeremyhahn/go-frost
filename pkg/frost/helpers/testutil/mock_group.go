...
func (m *MockGroup) NewScalar() group.Scalar {
	return &MockScalar{value: big.NewInt(0), order: m.order}
}
...
func (m *MockGroup) RandomScalar() (group.Scalar, error) {
	...
	return &MockScalar{value: val, order: m.order}, nil
}
...
func (m *MockGroup) DeserializeScalar(data []byte) (group.Scalar, error) {
	...
	return &MockScalar{value: value, order: m.order}, nil
}
...
func (s *MockScalar) Copy() group.Scalar {
	return &MockScalar{value: new(big.Int).Set(s.value), order: new(big.Int).Set(s.order)}
}
