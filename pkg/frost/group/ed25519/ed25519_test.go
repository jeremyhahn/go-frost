...
// TestNewEd25519Group tests group initialization.
func TestNewEd25519Group(t *testing.T) {
	g := NewGroup()
	if g == nil {
		t.Fatal("NewGroup returned nil")
	}
	if g.Name() != "ed25519" {
		t.Errorf("expected name 'ed25519', got '%s'", g.Name())
	}
...
func TestElementBytes(t *testing.T) {
	...
	// Identity bytes should be all zeros (except last byte = 1)
	id := g.Identity()
	idBytes := id.Bytes()
	if len(idBytes) != 32 {
...
func TestDeserializeElement(t *testing.T) {
	...
	// Test identity element (point (0,1))
	idBytes := make([]byte, 32)
	idBytes[0] = 1 // Ed25519 identity is (0,1)
...
func TestDeserializeScalar(t *testing.T) {
	...
	// Test invalid scalar (> group order) - should fail with canonical check
	invalidBytes := make([]byte, 32)
	for i := range invalidBytes {
		invalidBytes[i] = 0xFF
	}
	_, err = g.DeserializeScalar(invalidBytes)
	// Ed25519 uses canonical encoding, so this should fail
	if err == nil {
		t.Error("deserializing non-canonical scalar should return error")
	}
}
