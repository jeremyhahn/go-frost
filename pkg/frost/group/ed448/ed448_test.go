...
// TestNewEd448Group tests group initialization.
func TestNewEd448Group(t *testing.T) {
	g := NewGroup()
	if g == nil {
		t.Fatal("NewGroup returned nil")
	}
	if g.Name() != "ed448" {
		t.Errorf("expected name 'ed448', got '%s'", g.Name())
	}
	if g.ElementLength() != 57 {
		t.Errorf("expected element length 57, got %d", g.ElementLength())
	}
	if g.ScalarLength() != 57 {
		t.Errorf("expected scalar length 57, got %d", g.ScalarLength())
	}
}
...
func TestGroupOrder(t *testing.T) {
	...
	order := g.Order()
	if len(order) != 57 {
		t.Errorf("expected order length 57, got %d", len(order))
	}
...
func TestElementBytes(t *testing.T) {
	...
	bytes := gen.Bytes()
	if len(bytes) != 57 {
		t.Errorf("expected 57 bytes, got %d", len(bytes))
	}

	// Identity bytes should be encoded properly
	id := g.Identity()
	idBytes := id.Bytes()
	if len(idBytes) != 57 {
		t.Errorf("expected 57 bytes for identity, got %d", len(idBytes))
	}
}
...
// TestSubgroupValidation tests that elements are in the prime-order subgroup.
func TestSubgroupValidation(t *testing.T) {
	g := NewGroup()

	// Test that random elements are in the subgroup
	scalar, _ := g.RandomScalar()
	elem := g.ScalarBaseMult(scalar)

	// Serialize and deserialize should work (validates subgroup membership)
	...
	if !elem.Equal(elem2) {
		t.Error("round-trip should preserve element")
	}

	// Test that generator is in the subgroup
	gen := g.Generator()
	genBytes, err := g.SerializeElement(gen)
	if err != nil {
		t.Fatalf("failed to serialize generator: %v", err)
	}

	gen2, err := g.DeserializeElement(genBytes)
	if err != nil {
		t.Fatalf("failed to deserialize generator: %v", err)
	}

	if !gen.Equal(gen2) {
		t.Error("round-trip should preserve generator")
	}
}
